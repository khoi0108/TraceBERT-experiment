...
class Test:
    ...
    def read_artifacts(self, file_path, type):
        ...
        if type == 'commit':
            for _ , row in df.iterrows():
                self.commit_collection.add_commit(commit_id=row['commit_id'], summary=row['summary'], diff_added=row['diff_added'], 
            ...
            for _ , row in df.iterrows():
                self.issue_collection.add_issue(number=row['issue_id'], body=row['issue_desc'], comments=row['issue_comments'], 
    ...
    def store_artifact_text(self, issue_id_list, commit_id_list):
        ...
        for iss_id in issue_id_list:
            ...
            iss_text = f"{issue['issue_desc']} {issue['issue_comments']}"  
            self.issue_text_map[iss_id] = iss_text.strip()
        for cm_id in commit_id_list:
            commit = self.commits[cm_id]
            cm_text = f"{commit['summary']} ADDED_LINES {commit['diff_added']} REMOVED_LINES {commit['diff_removed']}"
